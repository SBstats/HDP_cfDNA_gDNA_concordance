...
###############################################################################
## slurm/run_krd_M1.R
## Fit M1 (tracking model) on KRd trial data — 4 parallel chains
## Saves: results/krd_M1_fit.rds
###############################################################################
...
cat(sprintf("KRd M1: p=%d, n=%d, N_obs=%d, T=%d\n",
            nrow(Y_g), model_data$n, model_data$N_obs, max(model_data$time)))

cat("\n=== Fitting M1 (Tracking, 4 chains) ===\n")
...
saveRDS(multi_M1, file.path(out_dir, "krd_M1_fit.rds"))
cat("\nM1 fit saved to results/krd_M1_fit.rds\n")
